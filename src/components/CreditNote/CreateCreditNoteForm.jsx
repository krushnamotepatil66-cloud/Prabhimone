...
function CreateCreditNoteForm({ editingCreditNote, onSave, onCancel }) {
  const { customers, creditNotes, invoices, addCustomer, settings, products, addProduct } = useApp();
  ...
  // Load editing credit note details if available
  useEffect(() => {
    if (editingCreditNote) {
      ...
      setForm({
        ...
        customerType: editingCreditNote.customerType || "Existing",
        mobileNumber: editingCreditNote.mobileNumber || "",
        gstin: editingCreditNote.gstin || "",
        email: editingCreditNote.email || "",
        billingAddress: editingCreditNote.billingAddress || "",
        state: editingCreditNote.state || "Maharashtra",
        placeOfSupply: editingCreditNote.placeOfSupply || "Maharashtra",
        creditNoteId: editingCreditNote.id,
        date: editingCreditNote.date,
        invoiceId: editingCreditNote.invoiceId || "",
        salesPerson: editingCreditNote.salesPerson || "",
        ...
        items: editingCreditNote.items && editingCreditNote.items.length > 0
          ...
          : [{ product: "Returned Goods Credit", description: "", hsn: "", qty: "", unit: "Nos", price: String(parseAmount(editingCreditNote.amount)), discount: "", discountType: "Flat", tax: "18" }],
        notes: editingCreditNote.notes || "",
        internalNote: editingCreditNote.internalNote || "",
        termsAndConditions: editingCreditNote.termsAndConditions || settings.creditNoteTermsAndConditions || defaultTermsAndConditions,
      });

      setAdditionalCharges(editingCreditNote.additionalCharges ? String(editingCreditNote.additionalCharges) : "");
      setAutoRoundOff(editingCreditNote.autoRoundOff !== undefined ? editingCreditNote.autoRoundOff : true);
      setStatus(editingCreditNote.status || "Open");
      ...
      setForm({
        ...
        creditNoteId: "",
        date: new Date().toISOString().split("T")[0],
        termsAndConditions: settings.creditNoteTermsAndConditions || defaultTermsAndConditions,
      });
      setStatus("Open");
      setIsEditingParty(true);
    }
  }, [editingCreditNote, creditNotes.length, settings.creditNoteTermsAndConditions]);
  ...
  const handleSubmitForm = (e) => {
    ...
    const savedCreditNote = {
      id: form.creditNoteId || `CN-${String(creditNotes.length + 1).padStart(3, "0")}`,
      ...
      <div className="new-invoice-header-row">
        <div className="new-invoice-header-left">
          <div className="breadcrumb-nav">
            <span>Home</span> &gt; <span>Credit Notes</span> &gt; <span className="active">Create</span>
          </div>
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <input
                              ...
                              placeholder="Refund/credit reason..."
                            />
                          </td>
                          <td>
                            <input
          ...
          <div className="new-invoice-card">
            ...
            <div className="card-body payment-fields-vertical">
              <div className="form-group">
                ...
                >
                  <option value="Open">Open (Unapplied)</option>
                  <option value="Closed">Closed (Applied/Refunded)</option>
                </select>
              </div>
      ...
      {showSettingsModal && (
        <div className="invoice-settings-popup-overlay">
          <div className="invoice-settings-popup">
            <h3>Credit Note Form Settings</h3>
