...
function CreateEstimateForm({ editingEstimate, onSave, onCancel }) {
  const { customers, estimates, addCustomer, settings , products, addProduct } = useApp();
  ...
  // Load editing estimate details if available
  useEffect(() => {
    if (editingEstimate) {
      ...
      setForm({
        ...
        customerType: editingEstimate.customerType || "Existing",
        mobileNumber: editingEstimate.mobileNumber || "",
        gstin: editingEstimate.gstin || "",
        email: editingEstimate.email || "",
        billingAddress: editingEstimate.billingAddress || "",
        state: editingEstimate.state || "Maharashtra",
        placeOfSupply: editingEstimate.placeOfSupply || "Maharashtra",
        paymentTerms: editingEstimate.terms || "Due on Receipt",
        estimateId: editingEstimate.id,
        date: editingEstimate.date,
        expiryDate: editingEstimate.expiryDate || editingEstimate.date,
        salesPerson: editingEstimate.salesPerson || "",
        referenceNo: editingEstimate.referenceNo || "",
        items: editingEstimate.items && editingEstimate.items.length > 0
          ...
          : [{ product: "Service Consultation", description: "", hsn: "", qty: "", unit: "Nos", price: String(parseAmount(editingEstimate.amount)), discount: "", discountType: "Flat", tax: "18" }],
        notes: editingEstimate.notes || "",
        internalNote: editingEstimate.internalNote || "",
        termsAndConditions: editingEstimate.termsAndConditions || settings.estimateTermsAndConditions || defaultTermsAndConditions,
      });

      setAdditionalCharges(editingEstimate.additionalCharges ? String(editingEstimate.additionalCharges) : "");
      setAutoRoundOff(editingEstimate.autoRoundOff !== undefined ? editingEstimate.autoRoundOff : true);
      setStatus(editingEstimate.status || "Draft");
      ...
      setForm({
        ...
        date: new Date().toISOString().split("T")[0],
        expiryDate: new Date().toISOString().split("T")[0],
        termsAndConditions: settings.estimateTermsAndConditions || defaultTermsAndConditions,
      });
      setStatus("Draft");
      setIsEditingParty(true);
    }
  }, [editingEstimate, estimates.length, settings.estimateTermsAndConditions]);
  ...
  const handleSubmitForm = (e) => {
    ...
    const savedEstimate = {
      id: form.estimateId || `EST-${String(estimates.length + 1).padStart(6, "0")}`,
    ...
    <div className="new-invoice-page-container">
      ...
      <div className="new-invoice-header-row">
        <div className="new-invoice-header-left">
          <div className="breadcrumb-nav">
            <span>Home</span> &gt; <span>Estimates</span> &gt; <span className="active">Create</span>
          </div>
          <h1 className="new-invoice-page-title">{editingEstimate ? `Edit Estimate (${form.estimateId})` : "Create Estimate"}</h1>
        </div>
      ...
      <form onSubmit={handleSubmitForm} className="new-invoice-grid-form">
        ...
        <div className="new-invoice-right-col">
          ...
          <div className="new-invoice-card">
            ...
            <div className="card-body flex-fields-vertical">
              <div className="form-group input-with-icon-group">
                <label className="required-field">Estimate Number</label>
                <div className="input-with-side-button">
                  <input 
                    type="text" 
                    value={form.estimateId}
                    onChange={(e) => handleInput("estimateId", e.target.value)}
                    placeholder="EST-000001"
              ...
              <div className="form-group">
                <label className="required-field">Estimate Date</label>
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  placeholder="e.g. REF-4587"
                />
              </div>
            </div>
          </div>

          {/* 2. Estimate Summary Card */}
          <div className="new-invoice-card">
            <div className="card-header">
              <span className="card-header-icon"><FiEdit3 /></span>
          ...
          {/* 3. Estimate Status & Validity Card */}
          <div className="new-invoice-card">
            <div className="card-header">
